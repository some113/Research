...
def main():
    ...
    file = get_filename_input("Enter path to file: ")
    ...
    if options['extract_ip']:
        options['ip_filter'] = get_ip_input("Source IP to filter (file 1):")
    if options['modify_ip']:
        options['subnet_map'] = get_subnet_mapping("File 1 subnet mapping")
    if ask_yes_no("Filter by time range in file 1?"):
        options['time_range'] = get_time_range("File 1 time range")
    if ask_yes_no("Shift timestamps in file 1?"):
        options['shift_time'] = True
        options['shift_seconds'] = get_time_shift("File 1 time shift")
    ...
    """
    processed = process_file(file, options)

    # === Merge and sort by timestamp ===
    merged = sorted(processed, key=lambda x: x[0])
