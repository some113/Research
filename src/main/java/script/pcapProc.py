...
input_file_path = "D:\Shared\\testOut.txt"  # Replace with your input file
output_file_path = "E:\Research\InputData\processedTestData.txt"  # Replace with your output file
        ...
        if key in startTimeDict:
            if time - startTimeDict[key] > TIME_GAP:
                ...
                with open(output_file_path, 'a') as outfile:
                    outfile.write(""+ str(time) + " " + key)
                    outfile.write(",".join(flowDict[key]) + "\n")
                    startTimeDict.pop(key)
                    flowDict.pop(key)
            else:
